...
function parseFrame(frame, stack) {
  // Parse the formatter that we can only hope CoffeeScript gave us. If it
  // doesn't match, we won't be impacted anyway.
  return frame.replace(/\((.+?\.coffee):(\d+):(\d+).+$/, function(match, file, line, col){
    // Only compile if the given line is the same as the original stack trace
    return match + fileContents(file, line, stack.getLineNumber() == line);
  });
}
  ...
  Error.prepareStackTrace = function(err, stack) {
    ...
    if (formatter) {
      ...
      return formatter.apply(this, arguments).split('\n').map(function(line){
        ...
        return parseFrame(line, stack.shift());
      }).join('\n');
    } else {
