...
var module = {
  _compile: function(compiled){ return compiled; }
};
...
function expandFrame(frame) {
  ...
  if (isCoffee) {
    ...
    try {
      ...
      if (fs.statSync(fileName).mtime > safeTime) {
        amend = color(["Source of " + path.basename(fileName) + " updated; cannot find line"]);
      } else {
        // So now we have a reliable source file. Get it and find the line.
        var readLine = lineNumber - 1;
        if (sources[fileName] === undefined)
          sources[fileName] = coffee(module, fileName);

        // Grab the context lines and color them
        var contents = sources[fileName].split('\n').slice(readLine - context, readLine + context + 1);
        amend = color(fixdent(contents), context);
      }
