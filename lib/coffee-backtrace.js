...
function parseFrame(frame) {
  ...
  return frame.replace(/\((.+?\.coffee):(\d+):(\d+)(, <js>)?.+$/, function(match, file, line, col, sourceMaps){
    // sourceMaps is only matched if CoffeeScript gave us the mapped lines
    return match + fileContents(file, line, !sourceMaps);
  });
}
